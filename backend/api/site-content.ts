...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const persisted = await fetchPersistedSiteContent();
    res.status(200).json(persisted ?? createDefaultPublicSiteContent());
  } catch (error) {
    console.error("[api/site-content] Failed to build site content:", error);
    res.status(200).json(createDefaultPublicSiteContent());
  }
}
